...
export default function TrackShowsPage() {
  ...
  const inProg = entries.filter((e) => e.episodeNumber > 0);
  const notStarted = entries.filter((e) => e.episodeNumber === 0);
                ...
                {inProg.map((e) => (
                  <Grid key={e.id} item xs={12} sm={6} md={4} lg={3}>
                    <TrackedShowCard
                      ...
                      onRemoved={(removedId) =>
                        setEntries((prev) =>
                          prev.filter((entry) => entry.id !== removedId)
                        )
                      }
                      onMutate={({ entryId, season, episode }) =>
                        setEntries((prev) =>
                          prev.map((x) =>
                            x.id === entryId
                              ? {
